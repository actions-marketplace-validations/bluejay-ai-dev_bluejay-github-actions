    ...
    try {
        ...
        while (true) {
            ...
            if (elapsedSeconds > timeoutSeconds) {
                throw new Error(`Timed out after ${timeoutSeconds}s waiting for simulation_run_id=${simulationRunId}`);
            }
            const statusResp = await fetch(`https://api.getbluejay.ai/v1/retrieve-simulation-results/${encodeURIComponent(simulationRunId)}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-API-Key': apiKey
            ...
            if (!statusResp.ok) {
                ...
                throw new Error(`Failed to fetch simulation status: HTTP ${statusResp.status}`);
            }
            let statusData;
            try {
            ...
            core.info(`Current simulation status=${lastStatus}, passed=${testsPassed}/${totalTests}, calculated_score=${score.toFixed(1)}`);
        ...
        if (normalizedStatus !== 'completed' && normalizedStatus !== 'success') {
            core.setFailed(`Bluejay simulation_run_id=${simulationRunId} ended with status=${lastStatus}`);
            return;
        }
        if (finalScore < minScore) {
            core.setFailed(`Bluejay overall_score=${finalScore} is below minimum threshold=${minScore}`);
        }
        else {
            core.info(`Bluejay overall_score=${finalScore} meets or exceeds threshold ${minScore}.`);
        }
    }
    catch (error) {
